...
class SegmentationPredictor:
    ...
    def __init__(self):
        ...
        if not hasattr(self, '_initialized'):
            ...
            logger.info("Initialized SegmentationPredictor")
    ...
    def tif_to_transformed_geojson(self, tif_file, geojson_file):
        ...
        try:
            ...
            # Use base predictor's transform_coordinates
    ...
    async def segment_with_text_prompt(self, *, bounding_box, text_prompt, box_threshold=0.3, text_threshold=0.3, zoom_level=20):
        ...
        try:
            self.text_predictor.download_satellite_image(image_name=input_image, bounding_box=bounding_box, zoom_level=zoom_level)
            logger.info(f"Downloaded satellite image.")
            await self.text_predictor.predict(input_image=input_image, text_prompt=text_prompt, box_threshold=box_threshold, text_threshold=text_threshold, output_image=output_image)
            logger.info(f"Predication finished.")            
                
            geojson = self.tif_to_transformed_geojson(tif_file=output_image, geojson_file=output_geojson)
    ...
    async def segment_with_points(self, zoom_level, box_threshold, points_include, points_exclude=None):
        ...
        try:
            ...
            self.point_predictor.download_satellite_image(image_name=input_image, bounding_box=bounding_box, zoom_level=zoom_level)
            await self.point_predictor.predict(input_image=input_image, points=points_include + (points_exclude or []), point_labels=point_labels, box_threshold=box_threshold, output_image=output_image)
            logger.info(f"Predication finished.")
